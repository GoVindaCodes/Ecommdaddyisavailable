...
const Category = () => {
  ...
  const handleSelectAll = () => {
    ...
    if (!isCheckAll) {
      const selectedIds = data.flatMap(category => {
        console.log("Category:", category);
        const categoryAndChildrenIds = [category._id];
        if (category.children && Array.isArray(category.children)) {
          const childIds = category.children.map(child => `${category._id}-${child}`);
          console.log("Child IDs:", childIds);
          categoryAndChildrenIds.push(...childIds);
        }
